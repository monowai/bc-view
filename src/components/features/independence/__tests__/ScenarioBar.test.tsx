...
describe("ScenarioBar", () => {
  it("renders all slider labels when expanded", () => {
  ...
  it("shows the Assumptions toggle label", () => {
    render(<ScenarioBar {...baseProps} />)
  ...
  describe("Expected Real Return slider", () => {
    it("is present in the slider grid when expanded", () => {
      render(<ScenarioBar {...baseProps} />)
      fireEvent.click(screen.getByRole("button", { name: /Show sliders/ }))
      expect(screen.getByText("Expected Real Return")).toBeInTheDocument()
    })

    it("defaults to the plan's blended real return when realReturn is null", () => {
      // planCashAlloc=1.0, planCashRate=0.015 → blended=1.5%, real=1.5%-2.5%=-1.0%
      render(<ScenarioBar {...baseProps} />)
      fireEvent.click(screen.getByRole("button", { name: /Show sliders/ }))
      const realReturnSlider = screen
        .getByText("Expected Real Return")
        .closest(".space-y-2")
        ?.querySelector('input[type="range"]') as HTMLInputElement
      // Default: planBlended(1.5%) - planInflation(2.5%) = -1.0% = -0.01
      expect(Number(realReturnSlider.value)).toBeCloseTo(-0.01, 5)
    })

    it("uses scenario.realReturn when set", () => {
      const props = {
        ...baseProps,
        scenario: { ...baseScenario, realReturn: 0.04 },
      }
      render(<ScenarioBar {...props} />)
      fireEvent.click(screen.getByRole("button", { name: /Show sliders/ }))
      const realReturnSlider = screen
        ...
        .closest(".space-y-2")
        ?.querySelector('input[type="range"]') as HTMLInputElement
      expect(Number(realReturnSlider.value)).toBeCloseTo(0.04, 5)
    })

    it("emits realReturn patch when slider changes", () => {
      render(<ScenarioBar {...baseProps} />)
      fireEvent.click(screen.getByRole("button", { name: /Show sliders/ }))
      const realReturnSlider = screen
        .getByText("Expected Real Return")
        .closest(".space-y-2")
        ?.querySelector('input[type="range"]') as HTMLInputElement
      fireEvent.change(realReturnSlider, { target: { value: "0.03" } })
      expect(baseProps.onScenarioChange).toHaveBeenCalledWith({
        realReturn: 0.03,
      })
    })
  ...
  describe("Cash → Investments education line", () => {
    ...
    it("shows the amber highlight box for Cash → Investments", () => {
      render(<ScenarioBar {...baseProps} />)
      fireEvent.click(screen.getByRole("button", { name: /Show sliders/ }))
      expect(
        screen.getByText(/Try it: move cash to investments/i),
      ).toBeInTheDocument()
    })
  })
})
